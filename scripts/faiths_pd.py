...
def args():
    ...
    parser.add_argument("--biom", help="Counts per ASV/OTU in BIOM format; names must correspond to leaves in the phylogeny passed to --tree")
    parser.add_argument("--out", help="Path to output file, will be written in TSV format")
    ...
    df = pd.DataFrame(
        {
            'names' : tab.index,
            'pd' : alpha_diversity(
                "faith_pd", tab, otu_ids=tab.columns, tree=tree
            ),
        }
    ).set_index('names')
